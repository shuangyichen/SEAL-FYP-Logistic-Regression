...
#define POLY_MOD_DEGREE 32768//16384
#define DEGREE 3
#define ITERS 2
...
Ciphertext relu(Ciphertext ct,  double scale, CKKSEncoder &ckks_encoder, Evaluator &evaluator, Encryptor &encryptor, RelinKeys relin_keys, EncryptionParameters params)
{
    vector<double> coeffs;
    coeffs = {0.00001, 1.0, 1.0};
    Ciphertext result;
    result = Tree_cipher(ct, coeffs.size() - 1, scale, coeffs, ckks_encoder, evaluator, encryptor, relin_keys, params);
    return result;
}
Ciphertext multiply_conv(Ciphertext ct, Plaintext pt, int size,int step,GaloisKeys gal_keys,Evaluator &evaluator,RelinKeys relin_keys,EncryptionParameters params, int out_H, int out_W, double scale, CKKSEncoder &ckks_encoder, Encryptor &encryptor)
{
    auto start_unit_layer = chrono::high_resolution_clock::now();
    Ciphertext result;
    evaluator.multiply_plain(ct,pt,result);
    evaluator.relinearize_inplace(result, relin_keys);
    evaluator.rescale_to_next_inplace(result);
    Ciphertext zero_filled;
    evaluator.rotate_vector(result, -size, gal_keys, zero_filled);
    Ciphertext dup;
    evaluator.add(result, zero_filled, dup);
     for (int i = 1; i < step; i++)
    {
        evaluator.rotate_vector_inplace(dup, 1, gal_keys);
        evaluator.add_inplace(result, dup);
    }
    SEALContext context(params);
    auto tmp = make_shared<SEALContext>(context);
    

    //print_Ciphertext_Info("temp", result, tmp);
    
    //masking
    vector<double> mask_vec(out_H*out_W*step, 0);
    for (int i=0; i<out_W*out_H; i++){
        for (int j=0; j<step; j++){
	    if (j==0){
	    mask_vec[step*i+j]=1.0;
	    }
	}
    }
    Plaintext mask_pt;
    ckks_encoder.encode(mask_vec, scale, mask_pt);
    evaluator.mod_switch_to_next_inplace(mask_pt);
    evaluator.multiply_plain_inplace(result, mask_pt);
    //print_Ciphertext_Info("temp", result, tmp);
    evaluator.relinearize_inplace(result, relin_keys);
    // Rescale
    evaluator.rescale_to_next_inplace(result);
    // Manual Rescale
    result.scale() = pow(2, (int)log2(result.scale()));
    print_Ciphertext_Info("temp", result, tmp);
    Ciphertext relu_res;
    relu_res = relu(result, scale, ckks_encoder,evaluator,encryptor,relin_keys,params);
     cout<<"Processing one unit layer" <<endl;
     auto stop_unit_layer = chrono::high_resolution_clock::now();
    auto duration_unit_layer = chrono::duration_cast<chrono::microseconds>(stop_unit_layer - start_unit_layer);
            cout << "Time:"<< duration_unit_layer.count() << " microseconds"<<endl;
    return relu_res;
}
/*
Ciphertext relu(Ciphertext ct,  double scale, CKKSEncoder &ckks_encoder, Evaluator &evaluator, Encryptor &encryptor, RelinKeys relin_keys, EncryptionParameters params)
...
*/
/*
Ciphertext rotate_vector_add(Ciphertext ct, int steps,Evaluator &evaluator,GaloisKeys gal_keys)
{
    for (int i=1; i< steps; i++)
    {
        Ciphertext temp;
        evaluator.rotate_vector(ct, i, gal_keys,temp);
	evaluator.add_inplace(ct,temp);
    }
    return result;
}
*/
Ciphertext add_inplace(Ciphertext ct1,Ciphertext ct2,Evaluator &evaluator)
{
    evaluator.add_inplace(ct1,ct2);
}

Ciphertext conv(vector<Ciphertext> features, Plaintext weights,Evaluator &evaluator, CKKSEncoder &ckks_encoder, double scale, GaloisKeys gal_keys, RelinKeys relin_keys, Encryptor &encryptor, EncryptionParameters params)
{
    SEALContext context(params);
    auto tmp = make_shared<SEALContext>(context);
    cout << "->" << __func__ << endl;
    ...
    for (int i = 0; i < num_rows; i++)
    {
        results[i] = multiply_conv(features[i], weights, 6084, 9,gal_keys,evaluator, relin_keys,params, 26, 26, scale, ckks_encoder, encryptor); 
        //evaluator.relinearize_inplace(results[i], relin_keys);
        //evaluator.rescale_to_next_inplace(results[i]);
        /*
	vector<Ciphertext> temp(8);
	for (int j = 1; j < 9; j++)
        {
            temp[j-1] =rotate_vector(results[i], 1, evaluator,gal_keys);
            //evaluator.add_inplace(mult, dup);
        }
	for (int k = 0; k < 8; k++)
	{
	    add_inplace(results[i], temp[k]);
	}
	results[i].scale() = pow(2, (int)log2(results[i].scale()));
        //cout << "\nMult Info:\n";
        //cout << "\tLevel:\t" << context->get_context_data(results[i].parms_id())->chain_index() << endl;
        //cout << "\tScale:\t" << log2(results[i].scale()) << endl;
        */
	//print_Ciphertext_Info("temp", results[i], tmp);
    }
    //print_Ciphertext_Info("temp,")
    return results[0];
}
...
Ciphertext update_weights(vector<Ciphertext> features, vector<Ciphertext> features_T, Ciphertext labels, Ciphertext weights, float learning_rate, Evaluator &evaluator, CKKSEncoder &ckks_encoder, GaloisKeys gal_keys, RelinKeys relin_keys, Encryptor &encryptor, double scale, EncryptionParameters params)
{
    ...
    //vector<Ciphertext> gradient_results(num_observarions);
    cout << "->" << __LINE__ << endl;
    //evaluator.mod_switch_to_inplace(features_T[i], pred_labels.parms_id());
    //    gradient_results[0] = cipher_dot_product(features[0], pred_labels, num_observations, relin_keys, gal_keys, evaluator);
    ...
    for (int i = 0; i < num_observations; i++)
    {
        ...
        evaluator.mod_switch_to_inplace(features[i], pred_labels.parms_id());
        gradient_results[i] = cipher_dot_product(features[i], pred_labels, num_observations, relin_keys, gal_keys, evaluator);
...
Ciphertext train_cipher(vector<Ciphertext> features, vector<Ciphertext> features_T, Ciphertext labels, Ciphertext weights, float learning_rate, int iters, int observations, int num_weights, Evaluator &evaluator, CKKSEncoder &ckks_encoder, double scale, GaloisKeys gal_keys, RelinKeys relin_keys, Encryptor &encryptor, Decryptor &decryptor, EncryptionParameters params)
{
    ...
    auto duration_encode_set1 = chrono::duration_cast<chrono::microseconds>(stop_encode_set1 - start_encode_set1);
            cout << "Time:"<< duration_encode_set1.count() << " microseconds"<<endl;
	    cout << "Weights:\n\t[";
...
int main()
{
    ...
    */
    ...
    // Read File
    string filename = "/home/shuangyi/shuangyi/SEAL-FYP-Logistic-Regression/mnist_test.csv";
    ...
    int filter_size = 3;
    int stride = 1;
    int F_H_out = (F_H-filter_size+1)/stride;
    int F_W_out = (F_W-filter_size+1)/stride;
    int filters = filter_size*filter_size;
    vector<vector<double>> features1(rows,vector<double>(F_H_out*F_W_out*filters));
    vector<vector<double>> features2(rows,vector<double>(F_H_out*F_W_out*filters));
    vector<double> weights(filters);
    
    for (int i = 0; i < F_H_out ;i++)
    {
        for (int j = 0; j < F_W_out; j++)
	{
		for (int k = 0; k < filter_size; k++)
		{
		    ...
		    {
		        if (j+n<3){
			    features1[0][(F_W_out*(i)+j)*filters+k*filter_size+n] = features[0][(i+k)*F_H+j+n];
			    features2[0][(F_W_out*(i)+j)*filters+k*filter_size+n] = 0;
			    features1[1][(F_W_out*(i)+j)*filters+k*filter_size+n] = features[1][(i+k)*F_H+j+n];
                            features2[1][(F_W_out*(i)+j)*filters+k*filter_size+n] = 0;
			}
			else{
			    features1[0][(F_W_out*(i)+j)*filters+k*filter_size+n] = 0;
			    features2[0][(F_W_out*(i)+j)*filters+k*filter_size+n] = features[0][(i+k)*F_H+j+n];
			    features1[1][(F_W_out*(i)+j)*filters+k*filter_size+n] = 0;
                            features2[1][(F_W_out*(i)+j)*filters+k*filter_size+n] = features[1][(i+k)*F_H+j+n];
			}
		    }
		}
	}
    ...
    for (int i = 0; i < rows; i++)
    {
        ...
        labels[i] = f_matrix[i][0];
    }

    int F1 = 10;
    int F2 = 774;
    vector<vector<double>> features1(rows, vector<double>(cols));
    vector<vector<double>> features2(rows, vector<double>(cols));
    for (int i = 0; i < rows; i++)
    {
        for (int j = 0; j < cols; j++)
        {
	    if (j<F1){
	        features1[i][j] = features[i][j];
		features2[i][j] = 0;
	    }
	    else{
	        features1[i][j] = 0;
	        features2[i][j] = features[i][j];
	    }
	}
    }

    for (int j=0;j<15;j++){
    	cout<<features1[0][j]<<endl;
	cout<<features2[0][j]<<endl;
    }
    */
    ...
    vector<double> weights_encoded(F_H_out*F_W_out*filters);
    ...
    for (int i = 0; i < F_H_out ;i++)
    {
        for (int j = 0; j < F_W_out; j++)
        {
                for (int k = 0; k < filter_size; k++)
                {
                    for (int n = 0; n < filter_size; n++)
                    {
                        weights_encoded[(F_W_out*(i)+j)*filters+k*filter_size+n] = weights[k*3+n];
                    }
                }
        }
    }
    ...
    // Features Print test
    cout << "Features1 row size = " << features1.size() << endl;
    cout << "Features1 col size = " << features1[0].size() << endl;
    cout << "Features2 row size = " << features2.size() << endl;
    cout << "Features2 col size = " << features2[0].size() << endl;

    //cout << "Labels row size = " << labels.size() << endl;
    cout << "Weights row size = " << weights_encoded.size() << endl;
    ...
    //vector<vector<double>> standard_features1 = standard_scaler_double(features1);
    //vector<vector<double>> standard_features2 = standard_scaler_double(features2);
    // Print old weights
    /*
    cout << "\nOLD WEIGHTS\n------------------"
         << endl;
    ...
    */
    ...
    auto start_set1 = chrono::high_resolution_clock::now();
    vector<Plaintext> features1_pt(features1.size());
    vector<Plaintext> features2_pt(features2.size());
    cout << "\nENCODING FEATURES ...";
    for (int i = 0; i < features.size(); i++)
    {
        ckks_encoder.encode(features1[i], scale, features1_pt[i]);
	ckks_encoder.encode(features2[i], scale, features2_pt[i]);
    }
    cout << "Done" << endl;
    /*
    ...
    */
    ...
    cout << "\nENCODING WEIGHTS...";
    ckks_encoder.encode(weights_encoded, scale, weights_pt);
    cout << "Done" << endl;
    ...
    */
    // -------------- ENCRYPTING ----------------
    //Encrypt features
    vector<Ciphertext> features1_ct(features.size());
    vector<Ciphertext> features2_ct(features.size());
    cout << "\nENCRYPTING FEATURES ...";
    for (int i = 0; i < features.size(); i++)
    {
        encryptor.encrypt(features1_pt[i], features1_ct[i]);
	 encryptor.encrypt(features2_pt[i], features2_ct[i]);
    }
    cout << "Done" << endl;
    /*	
    ...
    */
    ...
    cout << "Done" << endl;
    */
    //Ciphertext combined_feature;
     cout << "\nConcatnating festures from all clients--------------\n"
         << endl;
    for (int i = 0; i < features.size(); i++)
    {
    evaluator.add_inplace(features1_ct[i],features2_ct[i]);
    }
    // Encrypt labels
    /*
    ...
    cout << "Done" << endl;
    */
    //auto stop_set1 = chrono::high_resolution_clock::now();
    //auto duration_encode_set2 = chrono::duration_cast<chrono::microseconds>(stop_set1 - start_set1); 
    //cout << "Encoding & Encryption Time:"<< duration_encode_set2.count() << " microseconds"<<endl;
    // --------------- TRAIN ---------------
    //cout << "\nTraining--------------\n"
    //     << endl;
    ...
    //print_Ciphertext_Info("temp", , tmp);
    Ciphertext temp = features1_ct[0];
    auto tmp1 = make_shared<SEALContext>(context);
    print_Ciphertext_Info("temp", temp, tmp1);

    Ciphertext predictions;
    predictions = conv(features1_ct,weights_pt,evaluator,ckks_encoder,scale,gal_keys,relin_keys,encryptor,params);
    ...
    //Ciphertext new_weights = train_cipher(features1_ct, features1_ct, weights_pt, LEARNING_RATE, ITERS, observations, num_weights, evaluator, ckks_encoder, scale, gal_keys, relin_keys, encryptor, decryptor, params);
